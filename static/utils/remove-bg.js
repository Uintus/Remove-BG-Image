document.addEventListener('DOMContentLoaded', () => {
  ...
  const requiredElements = {
    originalColumn, removedBackgroundColumn, originalIndicator, removedIndicator,
    resultImg, loading, uploadArea, fileInput, uploadText, downloadBtn, backgroundUploadInput
  };
  ...
  function isValidDataUrl(url) {
    if (!url || typeof url !== 'string' || !url.startsWith('data:image/')) {
      console.error(`Invalid data URL: ${url ? url.slice(0, 50) : 'null or undefined'}...`);
      return false;
    }
  ...
  function showToast(message, type = 'success') {
    if (lastToastMessage !== message) {
      ...
      setTimeout(() => { lastToastMessage = ''; }, 3000);
    }
  }
  ...
  window.showImage = function(type, element) {
    ...
    if (type === 'original') {
      ...
      const src = completedImage && isValidDataUrl(completedImage) ? completedImage :
                  processedImage && isValidDataUrl(processedImage) ? processedImage : uploadedImage;
      resultImg.src = src;
  ...
  function removeBackground(imageData) {
    ...
    const timeoutId = setTimeout(() => {
      ...
      showToast('Error: Request timed out. Try a smaller image or check the server.', 'error');
    }, 30000);
    ...
    .catch((error) => {
      ...
      showToast(`Error: ${error.message || 'Server not responding'}`, 'error');
    });
  }
  ...
  function applyBackground(backgroundData) {
    ...
    formData.append('background', dataUrlToBlob(backgroundData), 'background_image.png');
    ...
    .then((data) => {
      ...
      if (data.status === 'success' && isValidDataUrl(data.data)) {
        ...
        showToast('Server failed to apply background. Applied locally.', 'warning');
      }
    })
    .catch((error) => {
      clearTimeout(timeoutId);
      ...
      applyColorClientSide(backgroundData, sourceImage);
      showToast(`Error: ${error.message || 'Server not responding. Applied background locally.'}`, 'warning');
    });
  }
    ...
    if (!prompt) {
      window.utils.showToast('Please enter a description for your background.', 'error');
      return;
    }

    if (isGenerating) {
      window.utils.showToast('Already generating a background, please wait.', 'warning');
      return;
    }
  ...
  function generateAIBackground(prompt) {
    ...
    .then(blob => {
      ...
      const reader = new FileReader();
      reader.onloadend = function() {
        generatedBackgroundUrl = reader.result;
    ...
    .catch(error => {
      ...
      window.utils.showToast(`Error: ${error.message || 'Failed to generate background'}`, 'error');
      closeModal();
    })
    .finally(() => {
      isGenerating = false;
  ...
  // Function to apply the background to the main image
  function applyBackground(backgroundUrl) {
    // First check if we have a processed image
    const sourceImage = localStorage.getItem('processedWithoutBackground');

    if (!sourceImage || !isValidDataUrl(sourceImage)) {
      window.utils.showToast('Please remove background first!', 'error');
      return;
    }

    // Show loading in the main image area
    if (currentTab === 'removed') {
      updateLoadingState(true);
    }
    ...
    const backgroundBlob = dataUrlToBlob(backgroundUrl);

    // Create FormData to send to the server
    const formData = new FormData();
    formData.append('image', sourceBlob, 'processed_image.png');
    formData.append('background', backgroundBlob, 'background_image.png');

    // Make API call to add background
    fetch('/add-background', {
      method: 'POST',
      body: formData
    })
    ...
    .then(blob => {
      ...
      reader.onloadend = function() {
        ...
        updateLoadingState(false);
        window.utils.showToast('Background applied successfully!', 'success');
      };
      reader.readAsDataURL(blob);
    })
    .catch(error => {
      ...
      updateLoadingState(false);
      window.utils.showToast(`Error: ${error.message || 'Failed to apply background'}`, 'error');

      // Try to apply background locally if server fails
      if (sourceImage && backgroundUrl) {
        applyColorClientSide(backgroundUrl, sourceImage);
      }
    });
  }
  ...
  function applyColorClientSide(backgroundData, sourceImage) {
    ...
    img.onload = () => {
      ...
      ctx.fillStyle = backgroundData.includes('base64') ? '#FFFFFF' : backgroundData;
  ...
  backgroundUploadInput.addEventListener('change', (e) => {
    ...
    if (file && ['image/png', 'image/jpeg'].includes(file.type)) {
      ...
      reader.onerror = () => showToast('Failed to read background image!', 'error');
      reader.readAsDataURL(file);
    } else {
  ...
  defaultBackgrounds.forEach(bg => {
    bg.addEventListener('click', () => applyBackground(bg.src));
  });

  // Color picker
  colorInputs.forEach(input => {
    input.addEventListener('click', (e) => {
      const color = e.target.tagName === 'INPUT' ? e.target.value : window.getComputedStyle(e.target).backgroundColor;
      const canvas = document.createElement('canvas');
